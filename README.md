...
- **传感器协议实现**：
  ...
  - 数据解析说明
    ...
    注：此解析方式适用于浓度和量程值读取命令（0x03：读取浓度，0x04：读取量程），零点值固定为0
...
2. **数据转换**：
   - 根据公式计算实际浓度：
     ```
     if (浓度值 <= SENSOR_ZERO_POINT) {
         输出值 = 0
     } else if (浓度值 >= 量程值) {
         输出值 = 4095
     } else {
         输出值 = (浓度值 - SENSOR_ZERO_POINT) * 4096 / 量程值
     }
     ```
...
2. **DAC7311控制**：
   - 使用GPIO模拟SPI时序
   - 时钟频率控制在30MHz以下
   - 16位数据传输（4位控制位 + 12位数据）（这个与实际不符）
   - AD5621正确的输入位移寄存器的帧结构为：
      - [15:14]PD1,PD0 电源管理位
      - [13:02] DAC的12位有效数据
      - [01:00]无用数据
